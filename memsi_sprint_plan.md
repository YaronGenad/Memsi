...
## Track D — לוח שליטה תפעולי (Control Dashboard)

ממשק נפרד לחלוטין מה-PyQt6 הקיים. שלושה חלונות עצמאיים שרצים על שלושה מסכים. טכנולוגיה: **Tauri + React** (frontend) + **FastAPI** (Python backend שמשתמש בקוד הקיים). אותו PostgreSQL.

הלוגיקה המרכזית: המסך האמצעי מציג רק בעיות לא-פתורות ממוינות לפי דחיפות. לחיצה על בעיה מסננת את הפאנלים הצדדיים (כוח אדם / מלאי) להציג רק את מה שרלוונטי לפתרונה. Drag & drop של נכס זמין לתוך בעיה = פתרון. כל יום הוא מסך עצמאי עם שני מצבים: **תמונת מצב** (קריאה בלבד) ו**מצב תכנון** (sandbox — שינויים לא מחייבים עד אישור).

**תלויות חיצוניות:** D1–D3 יכולים לרוץ במקביל ל-Tracks A/B/C. D4 ואילך דורשים B2+B3 (מלאי אמיתי).

---

### D1: תשתית — FastAPI + Tauri scaffold

**מטרה.** להקים את שלד הפרויקט החדש: FastAPI שחושף את נתוני הקיים, ואפליקציית Tauri עם שלושה חלונות.

**מה נכלל.**
- פרויקט חדש בתיקייה `dashboard/` בתוך הריפו הקיים.
  - `dashboard/backend/` — FastAPI app.
  - `dashboard/frontend/` — Tauri + React (TypeScript).
- FastAPI endpoints ראשוניים (read-only):
  - `GET /branches` — רשימת סניפים פעילים.
  - `GET /inventory/current` — מלאי נוכחי פר (סניף, קטגוריה) מ-PARTBAL / `local_inventory`.
  - `GET /inventory/min-stock` — מלאי מינימום פר (סניף, קטגוריה) מ-`min_stock_calculator`.
  - `GET /forecast/shortfalls?from_date=&to_date=` — תחזית חסרים לפי טווח תאריכים (מ-`forecast_predictions` + `min_stock`).
- שלושה חלונות Tauri: `window-staff`, `window-missions`, `window-inventory`. כל חלון — React app נפרד שנטען בחלון משלו.
- Shared state בין החלונות דרך Tauri IPC (`emit`/`listen`) — הבסיס ל-drag & drop בין חלונות.
- Dev environment: `uvicorn` לbackend, `tauri dev` לfrontend. סקריפט אחד `start_dashboard.sh / .bat` שמפעיל את שניהם.

**מה לא נכלל.** כל UI אמיתי — רק scaffolding. כל לוגיקת בעיות — ספרינט הבא.

**Definition of Done.** `start_dashboard.sh` פותח שלושה חלונות. לחיצת כפתור בחלון אחד מדליקה event שהחלון השני מציג. FastAPI מחזיר JSON תקין מ-`GET /branches`.

**הערכת זמן.** 3–4 ימי עבודה.

**תלויות.** A1 (pool), A2 (error handling). מומלץ גם B1 (sync לילי), אך לא חובה.

---

### D2: מנוע בעיות — חישוב חורים בזמן אמת

**מטרה.** לבנות את הלוגיקה שמייצרת את רשימת הבעיות: כל (סניף, קטגוריה, תאריך) שבו `מלאי_נוכחי < מלאי_מינימום` הוא חור.

**מה נכלל.**
- מודל נתונים חדש ב-PostgreSQL: טבלה `issues(id, issue_date, branch_code, category, issue_type, severity, status, resolution_note, resolved_at, created_at)`.
  - `issue_type`: `INVENTORY_SHORTAGE` / `STAFF_SHORTAGE` (להמשך).
  - `status`: `OPEN` / `PENDING` (כוכבית — בטיפול) / `RESOLVED`.
  - `severity`: מספר 1–10 (מחושב אוטומטית).
- מיגרציה חדשה `006_issues.sql`.
- לוגיקת חישוב `issue_engine.py`:
  - ריצה בכל sync לילי (B1) ובכל בקשת רענון ידנית.
  - לכל (סניף, קטגוריה): `gap = min_stock - current_stock`. אם `gap > 0` → צור issue אם לא קיים, עדכן severity.
  - severity מחושב לפי: גודל ה-gap יחסית למינימום + קצב צריכה (מ-`forecast_history`) + דחיפות הקטגוריה (hardcoded tier לעת עתה, ML בהמשך).
  - אם gap נסגר → סמן issue כ-`RESOLVED` אוטומטית.
- FastAPI endpoints:
  - `GET /issues?date=&status=` — בעיות לתאריך מסוים, אופציונלי לפי סטטוס.
  - `PATCH /issues/{id}/status` — עדכון סטטוס (OPEN→PENDING→RESOLVED) + הערת פתרון.
  - `GET /issues/dates` — אילו תאריכים עתידיים יש בהם בעיות חוזיות (לניווט ציר הזמן).

**מה לא נכלל.** UI — רק הbackend. בעיות כוח אדם — ספרינט D4.

**Definition of Done.** `GET /issues?date=today` מחזיר רשימת בעיות מלאי אמיתיות. ריצת `issue_engine.py` על נתוני הייצור מייצרת issues הגיוניים שאפשר לאמת ידנית מול `InventoryTab` הקיים.

**הערכת זמן.** 4–5 ימי עבודה.

**תלויות.** D1, B1 (נתוני מלאי מסונכרנים). B2+B3 רצויים אך לא חובה — אפשר להריץ על PARTBAL גולמי בינתיים.

---

### D3: מסך המשימות — לוח בעיות ויזואלי

**מטרה.** בניית חלון המשימות המרכזי: רשימת בעיות פתוחות ממוינות לפי severity, ניווט ציר זמן, ושני מצבי צפייה.

**מה נכלל.**
- **חלון `window-missions` (React):**
  - רשימת כרטיסי בעיות. כל כרטיס: סניף, קטגוריה, gap, severity (צבע: אדום/כתום/צהוב), סטטוס (OPEN / כוכבית PENDING / RESOLVED).
  - ממוין ברירת מחדל לפי severity יורד.
  - ניווט תאריכים: `← היום → מחר → מחרתיים...` — כל לחיצה טוענת issues של אותו תאריך.
  - **מצב תמונת מצב** (ברירת מחדל): כרטיסים בלבד, ללא עריכה.
  - **מצב תכנון** (toggle): כרטיסים הופכים ל-drop targets. שינויים מסומנים כ"טיוטה" עד אישור.
  - לחיצה על כרטיס בעיה → emit event לשני החלונות האחרים עם `{branch, category, date}` כ-filter context.
- **כפתור אישור במצב תכנון:** שולח את כל שינויי הטיוטה ב-batch ל-`PATCH /issues` ומחזיר למצב תמונת מצב. בעיות שנפתרו מסומנות בכוכבית.

**מה לא נכלל.** הפאנלים הצדדיים (D5). הגרירה בין חלונות (D5). כוח אדם (D4).

**Definition of Done.** פותחים את חלון המשימות, רואים בעיות מהיום ממוינות. לוחצים "מחר" — רואים תחזיות. לוחצים על בעיה — הכרטיס מתרחב עם פרטים. מעברי status עובדים.
...
### D4: מודל כוח אדם — טעינת סידור עבודה

**מטרה.** להכניס נתוני עובדים ומשמרות למערכת דרך Excel קיים, ולהוסיף מנגנון עדכון ידני לחריגים (מחלה, חופש).

**מה נכלל.**
- מודל נתונים חדש ב-PostgreSQL (מיגרציה `007_staff.sql`):
  - `employees(id, name, branch_code, roles TEXT[], is_active)` — roles = רשימת הכשרות (מנהל, מכירות, ממסי-טכנאי וכו').
  - `shifts(id, employee_id, shift_date, shift_type, branch_code, source)` — source: `excel` / `manual`.
  - `staff_exceptions(id, employee_id, from_date, to_date, exception_type, notes)` — exception_type: `SICK` / `VACATION` / `TRAINING` / `OTHER`.
- **Excel importer:** `staff_excel_importer.py` שמקבל את פורמט הסידור הנוכחי של מנהלי הסניפים ומייצר שורות ב-`shifts`. הלוגיקה: לזהות את הכותרות הנפוצות (שם עובד, תאריך, משמרת) ולמפות. גמיש לפורמטים שונים.
- **לוגיקת זמינות:** `staff_availability.py` — לכל (עובד, תאריך) מחזיר `AVAILABLE` / `ASSIGNED` / `UNAVAILABLE` בהתאם ל-shifts + exceptions.
- FastAPI endpoints:
  - `POST /staff/import-excel` — טעינת קובץ Excel → parse → insert shifts.
  - `GET /staff/availability?date=&branch=` — זמינות כוח אדם לתאריך וסניף.
  - `POST /staff/exceptions` — הוספת חריג (מחלה, חופש).
  - `DELETE /staff/exceptions/{id}` — מחיקת חריג.
- **חיבור ל-issue engine:** הרחבת `issue_engine.py` לזהות גם `STAFF_SHORTAGE` — משמרת ללא כיסוי מספיק → issue.

**מה לא נכלל.** ממשק הזנה לעובדים מהסניפים — מנהלי הסניפים ממשיכים לשלוח Excel במייל. הוספה/עריכת עובדים דרך UI — בשלב ראשון ישירות מה-`UpdatesTab` הקיים ב-PyQt6 (מוסיפים tab).

**Definition of Done.** טעינת Excel סידור עבודה → `GET /staff/availability?date=today` מחזיר נתונים נכונים. הוספת חריג "מחלה" ל-עובד X → issue חדש מופיע ב-`GET /issues`.

**הערכת זמן.** 5–7 ימי עבודה.

**תלויות.** D2 (issue engine). לא תלוי ב-D3 (אפשר לפתח במקביל).

---

### D5: פאנלים צדדיים + Drag & Drop בין חלונות

**מטרה.** בניית שני הפאנלים הצדדיים (כוח אדם / מלאי) וחיבורם למסך המשימות דרך drag & drop.

**מה נכלל.**
- **חלון `window-staff` (React):**
  - שלושה sections: זמין / מוקצה / לא זמין.
  - כשאין filter פעיל: מציג את כל העובדים ב-תאריך הנוכחי.
  - כשיש filter פעיל (לחיצה על בעיה במסך האמצעי): מציג רק עובדים עם ההכשרות הרלוונטיות לאותה משימה, ממוין — זמינים קודם, אחריהם "ניתן לשחרר ממשימה אחרת".
  - כל כרטיס עובד הוא **draggable** (HTML5 Drag API).

- **חלון `window-inventory` (React):**
  - שלושה sections: זמין (מרלוג + עודפים בסניפים אחרים) / מוקצה (בדרך, הוזמן) / לא זמין.
  - כשיש filter פעיל: מציג רק הקטגוריה הרלוונטית לבעיה, ממוין לפי כמות זמינה.
  - כל שורת מלאי היא **draggable**.

- **Drag & Drop בין חלונות:**
  - גרירת כרטיס (עובד או מלאי) מחלון צדדי → שחרור על כרטיס בעיה במסך האמצעי.
  - מימוש: Tauri IPC — `dragstart` מ-emit event עם payload, `drop` ב-window-missions קולט ומבצע `PATCH /issues/{id}` עם הנכס המשובץ.
  - **במצב תמונת מצב:** drag & drop מושבת.
  - **במצב תכנון:** drag & drop פעיל, שינויים מסומנים כטיוטה.

- FastAPI endpoints נוספים:
  - `GET /inventory/available?category=&exclude_branch=` — מה זמין לקטגוריה מחוץ לסניף הבעייתי (מרלוג + סניפים עם עודף).
  - `POST /issues/{id}/assign` — שיבוץ נכס (עובד / מלאי) לבעיה + עדכון סטטוס ל-PENDING.

**מה לא נכלל.** פעולות ERP אוטומטיות (שליחת הזמנה לפריוריטי). הפעולה היא תיעוד ההחלטה בלבד.

**Definition of Done.** גוררים כרטיס עובד מחלון שמאלי לבעיה במרכז → הבעיה עוברת ל-PENDING עם כוכבית. גוררים שורת מלאי מחלון ימני → אותו דבר. שלושת החלונות מסונכרנים בזמן אמת.

**הערכת זמן.** 7–9 ימי עבודה (הספרינט הכי מורכב מבחינת frontend).

**תלויות.** D3, D4.

---

### D6: תחזיות בעיות עתידיות + דירוג ML

**מטרה.** להרחיב את לוח השליטה לפעול קדימה בזמן — בעיות עתידיות חוזיות — ולדרג אותן אוטומטית לפי עדיפות.

**מה נכלל.**
- **בעיות חוזיות:** הרחבת `issue_engine.py` לייצר issues לתאריכים עתידיים:
  - `forecast_predictions` (מ-C2) + `min_stock` → gap חוזי לכל (סניף, קטגוריה, חודש).
  - issue עתידי מסומן `predicted: true` + `confidence` (0–1) מה-forecast interval.
  - issues עתידיים מתעדכנים בכל ריצת תחזית חדשה.
- **ניווט עתידי:** חלון המשימות מאפשר ניווט עד 30 ימים קדימה. ימים עם בעיות חוזיות — מסומנים בנקודה בציר הזמן.
- **דירוג severity משופר (ML קל):**
  - מודל `severity_ranker.py` — `RandomForestClassifier` שמדרג בעיות לפי: קצב צריכה, עומק gap, זמן עד צפי חסר, היסטוריית פתרון (כמה זמן לקח לפתור בעיות דומות בעבר).
  - אימון על-פי `issues` היסטוריות (אחרי שנצבר מספיק נתונים — לפחות 30 ימי usage).
  - בינתיים (לפני צבירת נתונים): severity rule-based לפי `gap / min_stock × category_weight`.
- **category_weight table:** טבלה קטנה `category_priority(category, weight)` — עורכים ידנית דרך PyQt6 UpdatesTab. קלאסית גדולה קשיחה = 10, בינונית מותג רכה = 3, וכו'.

**מה לא נכלל.** מודל ML מורכב לפני שיש נתוני usage אמיתיים. בשלב הראשון rule-based מספיק ומדויק יותר ממודל מאומן על מעט נתונים.

**Definition of Done.** פותחים את לוח השליטה ורואים נקודות על ציר הזמן לימים הקרובים. לוחצים על "עוד שבוע" ורואים בעיות חוזיות עם confidence. severity מחושב אוטומטית ומשקף נכון את העדיפויות העסקיות.

**הערכת זמן.** 5–6 ימי עבודה.

**תלויות.** D3, D5, C2 (תחזיות per-cell).
...
- **Track D:** 29–37 ימי עבודה (~7–9 שבועות)
- **סה"כ:** 76–98 ימי עבודה. Track D יכול לרוץ חלקית במקביל ל-B/C — D1–D4 לא חייבים לחכות לסיום B/C.

**מפת תלויות Track D:**
```
D1 (scaffold)
 └─ D2 (issue engine)
      ├─ D3 (missions screen)     ← אפשר במקביל ל-D4
      └─ D4 (staff model)         ← אפשר במקביל ל-D3
           └─ D5 (drag & drop)    ← דורש D3 + D4
                └─ D6 (forecasts + ML ranking) ← דורש D5 + C2
```
